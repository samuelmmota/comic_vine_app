...
@HiltViewModel
class IssueViewModel @Inject constructor(private val repository: IssuesRepository, private val sharedPreferences: SharedPreferences) : ViewModel() {
    ...
    val allIssues = repository.allIssues(_sortSettings?.toSortStringFormat()).cachedIn(viewModelScope)
    val allIssuesUi =
        repository.allIssues(_sortSettings?.toSortStringFormat()).map { it.map { it.toComicIssueUi() } }.cachedIn(viewModelScope)
}